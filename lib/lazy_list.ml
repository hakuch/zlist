...
let%test _ =
  test_elems
    (concat (elems [1; 2; 3]) (elems [4; 5; 6]))
    [1; 2; 3; 4; 5; 6]
...
let%test _ = test_elems (elems [1; 3; 2; 5] |> drop_while (fun x -> x <> 2)) [2; 5]
...
let%test _ =
  test_elems
    (continually 0 |> map (fun x -> x + 1) |> take 3)
    [1; 1; 1]
...
let%test _ =
  test_elems
    (continually (elems [1; 2; 3]) |> flatten |> take 5)
    [1; 2; 3; 1; 2]
...
let%test _ =
  ...
  test_elems
    (zip_all_with f (elems [2; 3]) (elems [10; 20; 30]))
    [12; 23; -1]
...
let%test _ =
  test_elems
    (enum_from_to 0 10 |> map_filter (fun x -> if 10 * x < 50 then Some x else None))
    [0; 1; 2; 3; 4]
