...
(* [iterate] *)
let () =
  assert (strict (iterate 0 (fun x -> x + 1) |> take 5) = [ 0; 1; 2; 3; 4 ])
...
let () =
  assert (
    strict (items [ 1; 2; 3; 4; 5 ] |> take_while (fun x -> x <> 3)) = [ 1; 2 ])
...
let () =
  assert (
    strict (items [ 1; 3; 2; 5 ] |> drop_while (fun x -> x <> 2)) = [ 2; 5 ])
...
let () =
  assert (
    strict (continually (items [ 1; 2; 3 ]) |> flatten |> take 5)
    = [ 1; 2; 3; 1; 2 ])
...
let () =
  assert (
    strict (concat (items [ 1; 2; 3 ]) (items [ 4; 5; 6 ]))
    = [ 1; 2; 3; 4; 5; 6 ])
...
let () =
  assert (
    strict
      (zip_with (fun x y -> x + y) (items [ 1; 2; 3 ]) (items [ 10; 20; 30 ]))
    = [ 11; 22; 33 ])
